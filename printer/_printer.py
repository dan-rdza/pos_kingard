...
import datetime
import os
from escpos.constants import RAS
from PIL import Image


class TicketPrinter:
    def __init__(self, printer_name=None, paper_width_mm=58, logo_path="assets/images/logo_.bmp"):
        """
    ...
    def _print_bmp_logo(self) -> bytes:
    ...
    def _print_logo(self) -> bytes:
        ...
        try:
            with Image.open(self.logo_path) as img:
                ...
                return self._pil_image_to_escpos(img)
                
        except Exception as e:
            print(f"Error: {e}")
            return b""

    def _pil_image_to_escpos(self, img: Image.Image) -> bytes:
        """Convierte imagen PIL a ESC/POS usando método estándar."""
        import escpos.image
        
        escpos_img = escpos.image.EscposImage(img)
        return b"\x1b\x61\x01" + escpos_img.to_raster_format()            
    ...
    def print_ticket(self, header: dict, items: list, totals: dict, payment_method: str, business: dict):
        ...
        # ---------- Encabezado ----------
        lines += self._text_line("Prescolar", align="center", bold=True,)
        ...
        if business.get("phone"):
            lines += self._text_line(f"{business['phone']}", align="center")
