...
class POSFrame(ctk.CTkFrame):
    ...
    def _load_quick_products(self):
        ...
        products = self.product_repo.get_top_sold(limit=8)
        if not products:
            products = self.product_repo.get_pos_shortcuts(limit=8)

        if not products:
            ctk.CTkLabel(frame, text="No hay productos frecuentes configurados", text_color="gray70").pack(pady=5)
            return
    ...
    def _refresh_cart(self):
        ...
        for item in self.cart:
            ...
            ctk.CTkButton(
                row, text="🔄", width=30,
                command=lambda i=item, q=qty_var, p=price_var: self._update_item(i, q.get(), p.get())
    ...
    # ---------- Pago ----------
    def _on_pay(self):
        # Aquí pronto conectaremos con SaleRepo
        if not self.cart:
        ...
        # Placeholder: en futuro guardaremos venta y limpiaremos carrito
